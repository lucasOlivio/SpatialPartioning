...
bool FollowObject::Initialize(rapidjson::Value& document)
{
    ...
    Value& objRotat = document["maxRotationSpeed"];
    isValid &= parser.GetFloat(objRotat, m_maxRotationSpeed);
...
{
    CommandGroup* pGroup = new CommandGroup();
    ...
    pOrient->Initialize(m_pTransform, m_pForce, true, m_lastTargetPosition, glm::vec3(m_maxRotationSpeed));
    pOrient->PreStart();
